...
if(-not $SkipToolPackageRestore.IsPresent) {
    ...
    if ( (-not (Test-Path $PACKAGES_CONFIG_MD5)) -Or
      ($md5Hash -ne (Get-Content $PACKAGES_CONFIG_MD5 )) ) {
        ...
        Remove-Item * -Recurse -Exclude packages.config,nuget.exe # remove installed tools (ie. cake)
    }

    Write-Verbose -Message "Restoring tools from NuGet..."
    $NuGetOutput = & $NUGET_EXE install $PACKAGES_CONFIG -ExcludeVersion -OutputDirectory `"$TOOLS_DIR`"
